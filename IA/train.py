...
class Training(object):
    ...
    def loading_data(self):
        ...
        train_transformations = Compose([
            ...
            CenterCrop(299),# Ajouter par Thibaut
            ToTensor(),
            normalize])# On retire la transformation aleatoire car il faudrait effectuer la même opération au flux
    ...
    def train(self, logger, epoch):
        ...
        #i = 0
        #(images, target) = self.train_loader[0]
        for i, (images, target, flux1, flux2) in enumerate(self.train_loader):
            ...
            # compute y_pred
            y_pred = self.model([image_var,flux_var])
    ...
    def validate(self, logger):
        ...
        for i, (images, labels, flux1, flux2) in enumerate(self.val_loader):
            # Former une seule matrice de flux
            flux = torch.cat((flux1,flux2),0)

            images = torch.autograd.Variable(images)
            labels = torch.autograd.Variable(labels)
            fluxs = torch.autograd.Variable(flux)
            ...
            if self.tencrop:
                ...
                output_batch_crops = self.model([images,fluxs])
                ...
                output_batch = self.model([images,images])
